...
public class LogUtils {
    private static String TAG = "SummaryOfHttpRequest";
    ...
    public final static void e(String message) {
        if (DEBUG <= LOG_ERROR) {
            ...
            Log.e(TAG, stackTrace.getClassName() + INIO + stackTrace.getLineNumber() + INIO + stackTrace.getMethodName()
                            + INFO + message);
        }
    }
    ...
    public final static void i(String message) {
        if (DEBUG <= LOG_INFO) {
            StackTraceElement stackTrace = new Throwable().getStackTrace()[1];
            Log.i(TAG, stackTrace.getClassName() + INIO + stackTrace.getLineNumber() + INIO + stackTrace.getMethodName()
                            + INFO + message);
        }
    }
